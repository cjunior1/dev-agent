...
def _is_safe(command: str) -> bool:
    # Collapse runs of whitespace so extra spaces/tabs can't slip a blocked
    # command past the substring check (e.g. "rm  -rf  /"). This is a
    # best-effort guardrail, not a security boundary.
    cmd_norm = " ".join(command.lower().split())
    return not any(blocked in cmd_norm for blocked in BLOCKED_COMMANDS)
